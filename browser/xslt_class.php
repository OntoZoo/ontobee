...
Purpose: Ontobee ontology browsing section xml ouput for ontology classes.
*/
...
$a_fields['query_key_sup_class_results'] = $fields;
...
$sup_class_results = parse_json_query($a_query_results['query_key_sup_class_results']);
...
foreach ($sup_class_results as $result) {
	$tmpSupClasses[$result['path']][]=$result;
}

if (!empty($tmpSupClasses)) {
	$sup_class_results=array_pop($tmpSupClasses);
	array_shift($sup_class_results);
}

foreach ($sup_class_results as $result) {
	$supClasses[$result['link']]=isset($result['label']) ? $result['label'] : getShortTerm($result['link']);
}

$supClasses = array_reverse($supClasses, true);
...
WHERE { 
{
?s rdfs:subClassOf <{$supClass_url}> .
FILTER (isIRI(?s)).
OPTIONAL {?s rdfs:label ?o} .
OPTIONAL {?sc rdfs:subClassOf ?s}
}
...
';
	...
	$a_fields['query_key_sib_class_results'] = $fields;
}
...
$sib_class_results = isset($a_query_results['query_key_sib_class_results']) ? parse_json_query($a_query_results['query_key_sib_class_results']) : array();
...
//			print_r($a_label);
	...
	for($i=0; $i<sizeof($a_term_type); $i++) {
		if (in_array($a_term_type[$i], array('Class', 'AnnotationProperty'))) {
...
<span style="color:#006600"><?php print(makeLink(UTF_to_Unicode($tmp_value))); ?></span>
<?php
			foreach ($annotation_of_annotation_results as $aa_result) {
...
<?php
		...
		if ($tmp_value!='') {
?>
<li><span style="color:#333333">seeAlso</span>: <span style="color:#006600"><?php print(makeLink($tmp_value)); ?></span>
<?php
			foreach ($annotation_of_annotation_results as $aa_result) {
...
<?php
		...
		if ($tmp_value!='') {
?>
<li><span style="color:#333333">comment</span>: <span style="color:#006600"><?php print(makeLink($tmp_value)); ?></span>
<?php
			foreach ($annotation_of_annotation_results as $aa_result) {
...
*/
	...
	if (isset($result_main[$settings['ns_owl'].'equivalentClass'])) {
?>
<div style="font-weight:bold">Equivalents</div>
<div style="background-color:#EAF1F2; border:#99CCFF 1px solid; margin-top:4px; margin-bottom:12px">
<ul>
<?php
		foreach ($result_main[$settings['ns_owl'].'equivalentClass'] as $equivalentClass) {
			$obj_equiv=getRDFjsonEquivalent($term_cbd, $equivalentClass['value'], $a_label, $link = true);
?>
<li>
...
<?php
	...
	if (!empty($supClasses) || !empty($subClasses)) {
?>

<div style="font-weight:bold"> Class Hierarchy</div>
<div style="background-color:#EAF1F2; border:#99CCFF 1px solid; margin-top:4px; margin-bottom:12px">
...
<?php 			
		if (!empty($supClasses)) {
			$supClass_url='';
			foreach ($supClasses as $supClass_url => $supClass_label) {
				if ($supClass_label=='') $supClass_label=getShortTerm($supClass_url);
				if ($supClass_url!='http://www.w3.org/2002/07/owl#Thing') {
?>
<li>+ <a oncontextmenu="return false;" href="/browser/rdf.php?o=<?php print($o); ?>&amp;iri=<?php print(myUrlEncode($supClass_url)); ?>"><?php print(UTF_to_Unicode($supClass_label)); ?></a>
<ul style="list-style-type:none; padding-left:0; margin-left:20px;">
...
<?php 	
			...
			$sibClasses=array();
			foreach ($sib_class_results as $result) {
				$sibClasses[$result['s']]=isset($result['o']) ? $result['o'] : '';
				if (isset($result['sc'])) $hasC[$result['s']]=true;
			}
			asort($sibClasses);

			
			if(!empty($sibClasses)) unset($sibClasses[$iri]);
			
			if(!empty($sibClasses)) {
//				if(sizeof($sibClasses)>50) $sibClasses = array_slice($sibClasses, 0, 50, true);
				$term_count=0;
				
				foreach ($sibClasses as $sibClass_url => $sibClass_label) {
					if ($sibClass_label=='') $sibClass_label=getShortTerm($sibClass_url);
					if (isset($hasC[$sibClass_url])) {
						$term_count++;
?>
<li>+<a oncontextmenu="return false;" href="/browser/rdf.php?o=<?php print($o); ?>&amp;iri=<?php print(myUrlEncode($sibClass_url)); ?>" <?php if ($sibClass_url==$iri) {?> style="color:#CC0000; font-size:larger" <?php } ?>><?php print(UTF_to_Unicode($sibClass_label)); ?></a>
</li>
<?php
					}
					
					if ($term_count>10) break;
				}
...
<?php
					...
					if ($term_count>20) {
?>
<li>+<a href="/browser/more.php?o=<?php print($o); ?>&amp;iri=<?php print(myUrlEncode($supClass_url)); ?>" style="color:#CC0000; font-size:120%">more... </a>
</li>
<?php
						break;
					}
...
<li>
<?php
			if (isset($hasC[$iri])) {?>+ <?php } else {?>- <?php }
?>
<a oncontextmenu="return false;" href="/browser/rdf.php?o=<?php print($o); ?>&amp;iri=<?php print(myUrlEncode($iri)); ?>" style="color:#CC0000; font-size:120%"><?php print($main_label); ?></a>
<?php
			if(!empty($subClasses)) {
?>
<ul style="list-style-type:none; padding-left:0; margin-left:20px;">
...
<?php
					...
					if ($term_count>20) {
?>
<li>+<a href="/browser/more.php?o=<?php print($o); ?>&amp;iri=<?php print(myUrlEncode($iri)); ?>" style="color:#CC0000; font-size:120%">more... </a>
</li>
<?php
						break;
					}
...
<li>
<?php
			if (!empty($subClasses)) {?>+ <?php } else {?>- <?php }
?>
...
<li>
<?php
				if (isset($hasC[$subClass_url])) {?>+ <?php } else {?>- <?php }
?>

<a oncontextmenu="return false;" href="/browser/rdf.php?o=<?php print($o); ?>&amp;iri=<?php print(myUrlEncode($subClass_url)); ?>"><?php print(UTF_to_Unicode($subClass_label)); ?></a></li>
<?php
			}
?>
</ul>
...
<?php
	...
	if (isset($result_main[$settings['ns_rdfs'].'subClassOf'])) {
?>
<div style="font-weight:bold">Superclasses &amp; Asserted Axioms</div>
<div style="background-color:#EAF1F2; border:#99CCFF 1px solid; margin-top:4px; margin-bottom:12px">
<ul>
<?php
		foreach ($result_main[$settings['ns_rdfs'].'subClassOf'] as $superclass) {
			if ($superclass['type']=='bnode') {
				$obj_equiv=getRDFjsonEquivalent($term_cbd, $superclass['value'], $a_label, $link = true);
				if (!empty($obj_equiv)) {
?>
...
?>
<li><a oncontextmenu="return false;" href="/browser/rdf.php?o=<?php print($o); ?>&amp;iri=<?php print(myUrlEncode($superclass['value'])); ?>"><?php print(UTF_to_Unicode($a_label[$superclass['value']]));?></a></li>
<?php
			}
		}
?>
...
?>
<li><a oncontextmenu="return false;" href="/browser/rdf.php?o=<?php print($o); ?>&amp;iri=<?php print(myUrlEncode($instance['s'])); ?>"><?php print( UTF_to_Unicode($instance['label'])); ?></a></li>
<?php
		}
?>
</ul>
